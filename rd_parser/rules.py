...
__all__ = ('BaseRule', 'Rule', 'Join', 'Choice', 'Repeat', 'Option', 'Predicate', 'Terminal',
    ...
    'RegexError', 'PredicateError', 'EndOfStreamError')
...
class Repeat(BaseRule):
    ...
    def match(self, source, offset):
        ...
        while _max is None or count < _max:
            try:
                offset, new_nodes, error = self.rule.match(source, offset)
                nodes.extend(new_nodes)
                count += 1
            except RuleError as e:
...
class Terminal(BaseRule):
    ...
    def match(self, source, offset):
        ...
        if source.startswith(self.terminal, offset):
            ...
            raise TerminalError(offset, "terminal failed to match", self)
...
class Regex(BaseRule):
    ...
    def match(self, source, offset):
        ...
        if not result:
            raise RegexError(offset, "regex failed to match", self)
        match = result[0]
        offset += len(match)
        if self.ignore_token:
            return offset, [], None
...
def _skip_whitespace(source, offset):
    result = _match_all_whitespace.match(source, offset)
    if not result:
        return offset
    else:
